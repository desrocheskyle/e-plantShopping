...
import { addItem } from './CartSlice'; // named export, not default

function ProductList({ onHomeClick }) {
  const [showCart, setShowCart] = useState(false);
  const [showPlants, setShowPlants] = useState(false);
  const [addedToCart, setAddedToCart] = useState({}); // fixed typo

  const dispatch = useDispatch();

  // === Redux cart state (Task 4 requirement) ===
  const CartItems = useSelector((state) => state.cart.items);

  // === Lab's hint implementation ===
  const calculateTotalQuantity = () => {
    return CartItems ? CartItems.reduce((total, item) => total + item.quantity, 0) : 0;
  };

  const handleAddToCart = (product) => {
    // Lab hint: dispatch(addItem(product));
        ...
        <div style={styleObjUl}>
          ...
          <div>
            <a href="#" onClick={handleCartClick} style={styleA}>
              <h1 className='cart' style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {/* Cart Icon */}
                ...
                {/* Badge: total quantity from Redux */}
            ...
            <div key={index}>
              ...
              <div className="product-list">
                {category.plants.map((plant, plantIndex) => {
                  const disabled = addedToCart[plant.name] || (CartItems?.some(i => i.name === plant.name));
                  return (
                    <div className="product-card" key={plantIndex}>
                      <img className="product-image" src={plant.image} alt={plant.name} />
                      <div className="product-title">{plant.name}</div>
                      <div className="product-description">{plant.description}</div>
                      {/* plant.cost already includes '$' — don’t prefix another '$' */}
                      <div className="product-cost">{plant.cost}</div>
                      <button
                        className={`product-button${disabled ? ' disabled' : ''}`}
